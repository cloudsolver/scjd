  ...
  /**
   * Utility that helps in the layout of user interface components that use the <tt>SpringLayout</tt>.
   * Provides a generic way of taking a <tt>Container</tt> object and creating a form. in a standard grid
   * All spacing and sizing issues are encapsulated here.
   *
   * @author Rohit Sood
   * @version 1.1
   */
  public final class SpringLayoutUtilities
  {
      ...
      /**
       * This method is used to create a grid view for the form. All components in the container are
       * layout as specified in the row and columns. Padding and initial coordinates within the container
       * is provided for embellishments.
       *
       ...
       */
      public static void makeSpringGrid(Container container, int rows, int cols, int x, int y, int padX,
          int padY)
      {
          ...
          System.out.println("w=" +totalWidth+" h="+totalHeight);
      ...
      private static int resolveHeight(Container container, int rows, int cols, int padY, int startY)
      {
          ...
          for (int r = 0;r < rows;r++)
          {
              ...
              // System.out.println("Max row height for row " + r + " is " + maxRowHeight);
      ...
      /**
       * Sets the y coordinate of every component in the container. Sets height of the
       * <tt>SpringLayout.Constraints</tt>.
       *
       ...
       */
      private static void setMaxHeight(Container container, int cols, int maxRowHeight, int startY,
          int row)
      {
          for (int col = 0;col < cols;col++)
          {
              try
              {
                  final Component              comp = container.getComponent(getComponentID(row, col,
                              cols));
      ...
      private static int getMaxHeight(Container container, int cols, int r)
      {
          ...
          for (int c = 0;c < cols;c++)
          {
              try
              {
                  ...
                  final int     componentHeight = ((SpringLayout) container.getLayout()).getConstraints(comp)
                                                         .getHeight().getValue();
       ...
       */
      private static void setMaximumWidth(Container container, int rows, int cols, int maxColumnWidth,
          int startX, int c)
      {
          for (int r = 0;r < rows;r++)
          {
              try
              {
                  final Component              comp        = container.getComponent(getComponentID(r, c,
                              cols));
                  final SpringLayout.Constraints constraints = ((SpringLayout) container.getLayout()).getConstraints(comp);
      ...
      private static int getMaximumWidth(Container container, int rows, int cols, int c)
      {
          ...
          for (int r = 0;r < rows;r++)
          {
              ...
              {
                  final Component comp         = container.getComponent(getComponentID(r, c, cols));
                  final int     componentWidth = ((SpringLayout) container.getLayout()).getConstraints(comp)
                                                        .getWidth().getValue();
      ...
      /**
       * Calculates the preferred dimension of the component given a mode. It takes the mode based on the
       * mode constants and calculates the preferred size.
       *
