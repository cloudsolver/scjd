  ...
  /**
   * The factory that creates a launcher based on type requested.
   * The factory returns a reference to a sub-type of the <tt>Launcher</tt> interface.
   * Invoking the launch method will luanch the application in a mode specified by the type of the subclass.
   * @author Rohit Sood
   * @version 1.2
   */
  ...
  {
      /**
       * Creates and returns an instance of a <tt>Launcher</tt> sub-class based on type.
       * This will throw an <tt>IllegalArguementException</tt> if an unknown mode is requested.
