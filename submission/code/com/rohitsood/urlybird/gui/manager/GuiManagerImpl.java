...
import com.rohitsood.urlybird.gui.record.ManageRecordInternalFrame;
...
import com.rohitsood.urlybird.gui.search.SearchRecordInternalFrame;
...
import com.rohitsood.urlybird.gui.server.ServerConsoleDataPanel;
import com.rohitsood.urlybird.gui.server.ServerConsoleInternalFrame;
...
import com.rohitsood.urlybird.server.DatabaseNetworkServerStarter;
...
class GuiManagerImpl implements GuiManager {
    ...
    /** The internal frame that lets the administrator see log messages. */
    private ServerConsoleInternalFrame serverConsoleInternalFrame;
    ...
    /** The dialog that displays programmer and application information */
    private AboutDialog aboutDialog;

    /** The mode the gui manager is expected to start in */
    private int mode;

    /** The server mode item that is registered. */
    private ServerModeMenuItem serverModeMenuItem;

    /**
     * Creates a new GuiManagerImpl object.
     */
    GuiManagerImpl(int mode) {
        setMode(mode);
    }
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.gui.GuiManager#setMode(int)
         */
    public void setMode(int mode) {
        this.mode = mode;
        client = new DatabaseClientFactory().getDatabaseClient(mode);
    }
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#setAboutDialog(com.rohitsood.urlybird.client.gui.widget.dialog.AboutDialog)
         */
    ...
    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.client.gui.GuiManager#openServerConsoleInternalFrame()
     */
    public void openServerConsoleInternalFrame() {
        ServerConsoleInternalFrame iframe = ServerConsoleInternalFrame.getInstance(this);

        if (iframe == serverConsoleInternalFrame) {
            iframe = null;
            serverConsoleInternalFrame.toFront();
        } else {
            serverConsoleInternalFrame = iframe;
        }
    }
    ...
    public void exitApplication() {
        final int option = JOptionPane.showConfirmDialog(window, "Are you sure ?", "Confirm Exit", JOptionPane.YES_NO_OPTION);
    ...
    /**
     * Handles the request to handle an exception. An exception option pane is displayed with the message contained in
     * the exception.
     *
     ...
     * @param message The detailed message to display.
     */
    public void handleInformation(String headline, String message) {
        ExceptionOptionPane.showInformation(window, headline, message);
    }
    ...
    public void goButtonClicked() {
        ...
        try {
        	...
        	handleException("Error parsing record number.", nfe);
        }
        catch (Exception e) {
            handleException(e);
        }
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#updateButtonClicked()
         */
    public void updateButtonClicked() {
        ...
        try {
            ...
            handleException("Error occured updating record",e);
        }
    }

    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#cancelButtonClicked()
         */
    ...
    /**
     * Clears the text field on the go panel.
     */
    private void clearGoField() {
        goPanel.getGoTextField().setText("");
    }

    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#editButtonClicked()
         */
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#deleteButtonClicked()
         */
    public void deleteButtonClicked() {
        try {
            ...
            handleException("Not a valid record id for delete.", nfe);
        } catch (Exception e) {
            handleException(e);
        }
    }
    ...
    public void savePreferencesButtonClicked() {
        try {
            preferencesDialog.persistProperties();
            handleInformation("Preferences Saved.", " The settings will take effect next time the application is started.");
            preferencesDialog.dispose();
        } catch (IOException e) {
            handleException(e);
        }
    ...
    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.gui.manager.GuiManager#setServerModeMenuItem(com.rohitsood.urlybird.gui.server.ServerModeMenuItem)
     */
    public void setServerModeMenuItem(ServerModeMenuItem serverModeMenuItem) {
        this.serverModeMenuItem = serverModeMenuItem;
    }
    ...
    public void confirmReservationButtonClicked() {
        ...
        try {
            confirmed = client.book(confirmBookingDialog.getRecordId(), confirmBookingDialog.getMaxOccupancy(), confirmBookingDialog.getCustomerId());
        } catch (Exception e) {
            confirmed = false;
            handleException(e);
        }
        ...
        if (confirmed) {
            ExceptionOptionPane.showInformation(getMainWindow(), "Confirmed.", "The booking was successful.");
            confirmCloseReservationButtonClicked();
        }
    }
    ...
    public void workOnline() {
        try {
            ...
            if ((null != getMainWindow()) && (null != modeMenuItem)) {
                getMainWindow().setTitle(ClientMainWindow.ONLINE_TITLE);
                modeMenuItem.setOnline(true);
            }
        } catch (Exception e) {
            handleException("There was a problem connecting to the remote server.\n" + "Please modify the remote server settings by going to Preferences.\n" + "Switching to Offline Mode.\n", e);
            workOffline();
        }
    }
    ...
    public void workOffline() {
        try {
            ...
            if ((null != getMainWindow()) && (null != modeMenuItem)) {
                getMainWindow().setTitle(ClientMainWindow.OFFLINE_TITLE);
                modeMenuItem.setOnline(false);
            }
        } catch (Exception e) {
            handleException("There was a problem working in Offline Mode.\n Please check Preferences.\n", e);
        }
    }

    /**
     * Gets a database client. The client can be either a network client or a local client depending on the mode of the
     * application.
     *
    ...
    /**
     * Sets the database client. The client can be either a network client or a local client depending on the mode of
     * the application.
     *
     ...
     */
    public void setMainWindow(AbstractMainWindow window) {
        this.window = window;
    }
    ...
    /* (non-Javadoc)
       * @see com.rohitsood.urlybird.client.gui.GuiManager#closeAboutDialog()
       */
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#openAboutDialog()
         */
    ...
    /* (non-Javadoc)
         * @see com.rohitsood.urlybird.client.gui.GuiManager#appendToConsole(java.lang.String)
         */
    public void appendToConsole(String message) {
        ServerConsoleDataPanel.appendToConsole(message);
    }

    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.client.gui.GuiManager#clearServerConsole()
     */
    public void clearServerConsole() {
        ServerConsoleDataPanel.clearConsole();
    }

    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.gui.GuiManager#startServer()
     */
    public void startServer() {
        openServerConsoleInternalFrame();

        DatabaseNetworkServerStarter starter = DatabaseNetworkServerStarter.getInstance(this);

        try {
            starter.start();
            serverModeMenuItem.setStarted(true);
        } catch (Exception e) {
            serverModeMenuItem.setStarted(false);
            handleException(e);
        }
    }

    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.gui.GuiManager#stopServer()
     */
    ...
    /* (non-Javadoc)
     * @see com.rohitsood.urlybird.gui.GuiManager#closeServerConsole()
     */
    public void closeServerConsole() {
        serverConsoleInternalFrame.dispose();
    }
}
