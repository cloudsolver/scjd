...
/**
 * Remote database server supports all data access funtions required  by the system. As defined by <tt>DBMain</tt> this
 * permits clients to manage the data records.  Because this is a remote object which must throw
 * <tt>RemoteException</tt>s on every method, this does not extends from <tt>DBMain</tt> which does not throw
 * <tt>RemoteException</tt>s. Instead all methods defined in <tt>DBMain</tt> are defined here verbatim. In addition
 * this also permits the user to book a room.
 *
 * @author Rohit Sood
 * @version 1.4
 */
public interface DatabaseNetworkServer extends Remote {
    /**
     * Books a room for the customer.  Validation is made based on the current system date, customer id and max
     * occupancy.
     *
     ...
     */
    boolean book(String recId, String maxOccupancy, String customerId) throws RecordNotFoundException, ValidationException, RemoteException;
     ...
     */
    void update(int recordNumber, String[] data) throws RecordNotFoundException, RemoteException;
    ...
    /**
     * Locks a record so that it can only be updated or deleted by this client. If the specified record is already
     * locked, the current thread gives up the CPU and consumes no CPU cycles until the record is unlocked.
     *
